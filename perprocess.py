...
def std_col_names(pd_dataframe):
	...
	pd_dataframe.columns = [col.replace('famcode', 'family_type')for col in pd_dataframe.columns]
	# if dataframe headers have release_year replace with year
	pd_dataframe.columns = [col.replace('release_year', 'year')for col in pd_dataframe.columns]
	# if dataframe headers have release_year replace with place
	pd_dataframe.columns = [col.replace('town', 'place')for col in pd_dataframe.columns]
	# if dataframe headers have county replace with place
	pd_dataframe.columns = [col.replace('county', 'place')for col in pd_dataframe.columns]
	# if dataframe headers have status replace with place
	pd_dataframe.columns = [col.replace('status', 'analysis')for col in pd_dataframe.columns]
	# if dataframe headers have status replace with place
	pd_dataframe.columns = [col.replace('&', 'and')for col in pd_dataframe.columns]
	return pd_dataframe
